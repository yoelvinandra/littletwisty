...
class LaporanUser extends MY_Controller {
	...
	{		
		...
		//PERUSAHAAN
		$wherePerusahaan = "and MUSER.idperusahaan=$perusahaan";

		//STATUS
		$whereStatus .= "and MUSER.STATUS LIKE '%".$jenisStatus[0]."%'";

		$sql = "select * from muser where (1=1 $whereFilter) $wherePerusahaan $whereStatus order by username asc";
		$data['sql']      = $sql;
		$data['excel']    = $this->input->post('excel');
		$data['filename'] = $this->input->post('file_name');
		
		$dir = 'reports/v_';
		$this->load->view($dir."report_master_user.php",$data);
	}
}
