...
class Tiktok extends MY_Controller {
	...
	public function getKategori(){
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://".$_SERVER['SERVER_NAME'] ."/pos/shopee/getAPI/",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => '',
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
        ...
        else
        {
            ...
            foreach($respLogistics as $itemLogistics)
            {
                if($itemLogistics['enabled'] && $itemLogistics['mask_channel_id'] == 0)
                {
                    ...
                    {
                        if($itemLogistics['enabled'] && $itemDetailLogistics['mask_channel_id'] == $itemLogistics['logistics_channel_id'])
                        {
	...
	function setBarang(){
		...
		$idBarang           = $this->input->post("IDBARANGSHOPEE",0);
		...
		$parameter = [];
		$parameter['original_price']    = (int)$dataVarian[0]->HARGAJUAL;
		...
		$parameter['category_id'] = (int)$this->input->post("KATEGORI");
		
		$parameter['image'] = array(
		    'image_id_list' => $dataGambarProduk
		);
		$parameter['item_sku'] = $dataVarian[0]->SKUSHOPEE;
		...
		$parameter['size_chart_info'] = array(
		    "size_chart"  => $this->input->post("SIZECHART"),
		    "size_chart_id" => (int)$this->input->post("SIZECHARTID")
		);
        ...
        curl_setopt_array($curl, array(
          CURLOPT_URL => "https://".$_SERVER['SERVER_NAME'] ."/pos/shopee/postAPI/",
          CURLOPT_RETURNTRANSFER => true,
          CURLOPT_ENCODING => '',
          CURLOPT_MAXREDIRS => 10,
          CURLOPT_TIMEOUT => 30,
        ...
        else
        {
            print_r($ret);
          	//TAMBAH VARIAN
            $parameter = [];
            $parameter['item_id'] = $ret['response']['item_id']??0;
            ...
            curl_setopt_array($curl, array(
              ...
              CURLOPT_CUSTOMREQUEST => 'POST',
              CURLOPT_POSTFIELDS =>  array(
              'endpoint' => 'product/init_tier_variation',
	...
	public function dispute(){
		...
		$parameter['dispute_reason_id'] = $disputeid;
	...
	public function getDispute(){
        ...
        else
        {
           ...
           {
               $response[$index]['dispute_text'] = $this->getAlasanDispute($index);
               $index++;
           }
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $destination)) {
            ...
            if($tipe == "GAMBAR")
            {
        	    ...
        	    if($response)
        	    {
                    ...
                    else
                    {
                       ...
                       $data['urlLocal'] = "https://".$_SERVER['SERVER_NAME'] ."/pos/assets/".$this->input->post('reason')."/". $kode . "_" . $index.$type;
            		   echo(json_encode($data));
                    }
           ...
           else if($tipe == "VIDEO")
           {
        	    ...
        	    if($response)
        	    {
                    ...
                    $ch = curl_init();
                    // Set cURL options
                    curl_setopt($ch, CURLOPT_URL,  $host.'?access_token='.$accessToken.'&timestamp='.$timest.'&sign='.$sign.'&shop_id='.$shopId.'&partner_id='.$partnerId); // destination URL
                    curl_setopt($ch, CURLOPT_POST, true);
                    curl_setopt($ch, CURLOPT_POSTFIELDS, ($postData));
                    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
                    curl_setopt($ch, CURLOPT_HTTPHEADER, [
	...
	public function print(){
        ...
        // $data['merge_url'] = "https://".$_SERVER['SERVER_NAME'] ."/pos/assets/label/merged.pdf";
        echo(json_encode($data));
	...
	public function getAlasanDispute($index){
	    $alasan =  [
	        "Tidak menerima pengembalian barang",
	        "Menerima pengembalian barang dengan kerusakan fisik",
	        "Menerima pengembalian barang yang tidak lengkap (jumlah / aksesoris hilang)",
	        "Menerima pengembalian barang yang salah",
	        "Pengembalian barang telah diterima, tetapi barang telah digunakan"
	    ];
