					 ...
					 <div class="col-md-12">
                        <div class="box-body" style="padding-left:0px; padding-right:0px;">
								...
								<div class="form-group col-md-5">
									...
									<div class="col-md-9 col-sm-9 col-xs-9"  style="padding: 0px 0px 5px 0px">
										<select  class="form-control select2"  id="NOPO" name="NOPO" style=" width:100% !important;">
											<option value="0 | -"> - Pilih - </option>
											<?=comboGridTransaksi("model_inventori_pesananbeli",["S"])?>
										</select>
									</div>
									<div class="col-md-3 col-sm-3 col-xs-3" style="padding: 0px">
										<label>S. Bayar </label>
								...
								<!--<div class="col-md-2 col-sm-2 col-xs-12">
									<input type="text" class="form-control" id="QTY" style="font-size:29pt; width:80%; text-align:right; 
						...
						<!-- DETAIL -->
							...
							<!--MODAL SUPPLIER-->
								<div class="modal fade" id="modal-supplier" >
									...
									<div class="modal-content">
										<div class="modal-body">
											<table id="table_supplier" class="table table-bordered table-striped table-hover display nowrap">
												<thead>
													<tr>
														<th></th>
														<th width="50px">Kode</th>
...
$(document).ready(function(){
    ...
    $("#btn_salin").click(function(){
        if($("#DETAILBARANG").val() != "")
        {
    	    $.ajax({
    			type    : 'POST',
    			url     : base_url+'Inventori/Transaksi/Pembelian/loadDetail/',
    ...
    $("#btn_tambah_all").click(function() {
	    ...
	    for(var i = 0; i < countdetail;i++)
    	{
    	    if(typeof $('#JUMLAH'+i+'  input').val() != "undefined") // BARANG TELAH DIHAPUS
		    {
            	...
            	for(var x = 0 ; x < dataPO.length ; x++)
            	{
            	    if(dataPO[x].KODE == $("#KODE"+i).html())
            	    {
            	        if(parseInt($("#JUMLAH"+i+" input").val()) > parseInt(dataPO[x].QTY))
            	        {
            	            adaKelebihan = true;
            	            $("#JUMLAH"+i+" input").val(dataPO[x].QTY);
            	        }
            	    }
            	}
		...
		{
            Swal.fire({
            	title            : "Terdapat Barang yang melebihi sisa PO.\nJumlah barang akan disesuaikan dengan sisa PO",
            	type             : 'success',
	...
	var urlAjax = base_url+'Master/Data/Barang/comboGridTransaksi';
	$("#NAMASUPPLIER").attr("readonly","readonly");
	$("#INFOSUPPLIER").attr("readonly","readonly");
// 	$("#SYARATBAYAR").attr("readonly","readonly");
	...
	$('#NOPO').change(function() {
		...
		if($('#NOPO').val() != "0 | -" && $("#mode").val() == "tambah")
		{	
		//HEADER
		 $.ajax({
			type    : 'POST',
			url     : base_url+'Inventori/Transaksi/PesananPembelian/loadHeader/',
			data    : {id:$('#NOPO').val().split(" | ")[0],mode:$("#mode").val()},
			dataType: 'json',
			success : function(msg){
			    $("#INPUTBARCODE,#DETAILBARANG,#btn_search,#btn_salin").css('filter', 'grayscale(100%)');
				$("#INPUTBARCODE,#DETAILBARANG,#btn_search,#btn_salin").attr('disabled','disabled');
	            $("#JMLMASSAL").val("");
			    var dateTGLTRANS = new Date(msg.TGLTRANS); // Get the date from #TGLTRANS
                var dateNow = new Date(); // Get the date from #tgl_awal_filter
                ...
                if (dateTGLTRANS && dateNow) {
                    if (dateTGLTRANS > dateNow) {
                       	Swal.fire({
                			title            : 'Tanggal PO yang dipilih adalah Maksimal Hari Ini',
                			type             : 'warning',
                			showConfirmButton: false,
                			timer            : 1500
                		});
            ...
            // 			$("#TGLTRANS").attr('disabled','disabled');
            			...
            			$.ajax({
            				type    : 'POST',
            				url     : base_url+'Inventori/Transaksi/PesananPembelian/loadDetailSisa',    // Master/Data/Barang/loadData
            				data    : {id:$('#NOPO').val().split(" | ")[0],mode:$("#mode").val()},
            				dataType: 'json',
            				success : function(msg){
            					...
            					for(var i = 0; i < msg.length;i++)
            					{
            						...
            						else if(pakaippn == "INCL")
            						{
            							...
            							{				
            								total+= parseFloat($('#SUBTOTAL'+i+'  input').val());
            								ppnrp[i] = Math.floor(parseFloat($('#SUBTOTAL'+i+'  input').val())/11).toFixed("<?=$_SESSION[NAMAPROGRAM]['DECIMALDIGITAMOUNT']?>");
            								ppn  += ppnrp[i];
            							}
		...
		{ 
		    $("#SYARATBAYAR,#TGLTRANS").removeAttr("disabled");
		    $("#LOKASI").removeAttr('disabled');
		    $("#INPUTBARCODE,#DETAILBARANG,#btn_search,#btn_salin").css('filter', '');
		    $("#INPUTBARCODE,#DETAILBARANG,#btn_search,#btn_salin").removeAttr('disabled');
		}
	...
	$('#dataGrid').DataTable({
		...
		ajax		  : {
			url    : base_url+'Inventori/Transaksi/Pembelian/dataGrid',
			dataSrc: "rows",
			type   : "POST",
			data   : function(e){
					e.status 		 = getStatus();
        ...
        columns:[
            { data: '' },    
            { data: 'IDBELI', visible: false,},	
            { data: 'TGLTRANS', className:"text-center"},	
            { data: 'KODEBELI', className:"text-center"},
	...
	$("#table_barang").DataTable({
		...
		"dom"		  : '<"pull-left"f><"pull-right"l>tip',
		ajax		  : {
			url    : urlAjax, // Master/Data/Barang/loadData
			dataSrc: "rows",
			type   : "POST",
...
function ubah(row){
	$("#JMLMASSAL").val("");
    $("#DETAILBARANG").val("");
	$("#DETAILBARANG").css('filter', '');
	$("#DETAILBARANG").removeAttr('disabled');
	get_akses_user('<?=$_GET['kode']?>', function(data){
		if (data.UBAH==1) {
			$("#mode").val('ubah');	
			...
			get_status_trans("Inventori/Transaksi/Pembelian",row.IDBELI,function(data){
				if(data.status != "I")
				{
				...
				if(data.status == "I")
				{
					...
					$("#NOPO").html('<option value="0 | -"> - Pilih - </option>');
					$.ajax({
                				type    : 'POST',
                				url     : base_url+'Inventori/Transaksi/PesananPembelian/comboGridTransaksi/',
                				data    : {status:["S","C"]},
                				dataType: 'json',
                				success : function(msg){
                				    var data = msg.rows;
                				    ...
                				    {
                				        $("#NOPO").append('<option value="'+data[x].VALUE+'">'+data[x].TEXT+'</option>');
                				    }
                				}
            	     });
				}
			...
			setTimeout(function() {	
    			//HEADER
    			 $.ajax({
    				type    : 'POST',
    				url     : base_url+'Inventori/Transaksi/Pembelian/loadHeader/',
    				data    : {id:row.IDBELI,mode:"ubah"},
    				dataType: 'json',
    				success : function(msg){			
    					...
    					$("#IDTRANS").attr('readonly','readonly');
    					$("#NOTRANS").attr('readonly','readonly');
    					$("#TGLTRANS").attr('disabled','disabled');
    	                $("#TGLJTHTEMPO").attr('disabled','disabled');
    					$("#IDSUPPLIER").attr('readonly','readonly');
    					$("#LOKASI").attr('disabled','disabled');
            			...
            			$.ajax({
            				...
            				success : function(msg){			
            						...
            						//$("#kode-tgl").html(row.KODEBELI+" &nbsp; ~ &nbsp; "+row.TGLTRANS+"<br>"+row.NAMASUPPLIER);
		...
		if (data.status=='I' || data.status=='S') {
			get_akses_user('<?=$kodemenu?>', function(data){
				if (data.HAPUS==1) {
					...
					$("#KETERANGAN_BATAL").html("Apa anda yakin akan membatalkan transaksi "+row.KODEBELI+" ?");
				} else {
					Swal.fire({
						title            : 'Anda Tidak Memiliki Hak Akses',
						type             : 'error',
...
function batal(){
	...
	if (row  && alasan != "") {
		$.ajax({
			type    : 'POST',
			dataType: 'json',
			url     : base_url+"Inventori/Transaksi/Pembelian/batalTrans",
			data    : "idtrans="+row.IDBELI + "&kodetrans="+row.KODEBELI + "&alasan="+alasan,
			cache   : false,
			success : function(msg){				
				if (msg.success) {
					Swal.fire({
						title            : 'Transaksi dengan kode '+row.KODEBELI+' telah dibatalkan',
						type             : 'success',
						showConfirmButton: false,
						timer            : 1500
					});
			...
			get_status_trans("Inventori/Transaksi/Pembelian",row.IDBELI, function(data){
				if (data.status =='I') {
					$.ajax({
						type    : 'POST',
						dataType: 'json',
						url     : base_url+'Inventori/Transaksi/Pembelian/ubahStatusJadiSlip',
						data    : {idtrans: row.IDBELI, kodetrans: row.KODEBELI},
						cache   : false,
						success : function(msg){
							if (msg.success) {
								$("#dataGrid").DataTable().ajax.reload();
								window.open(base_url+"Inventori/Transaksi/Pembelian/cetak/"+row.IDBELI, '_blank');
...
function simpan(){
	...
	{
		Swal.fire({
            title            : "Informasi Supplier harus diisi",
            type             : 'warning',
            showConfirmButton: false,
            timer            : 1500
		});
	...
	else
	{
		$.ajax({
			type    : 'POST',
			url     : base_url+'Inventori/Transaksi/Pembelian/simpan/', 
			data    : {	IDBELI		 : 	$("#IDTRANS").val(),
						KODEBELI     : $("#NOTRANS").val(),
						IDLOKASI     : $("#LOKASI").val(),
						IDSYARATBAYAR: $('#SYARATBAYAR').val().split(" | ")[3],
						IDSUPPLIER   : $("#IDSUPPLIER").val(),
						IDPO         : $('#NOPO').val().split(" | ")[0],
						KODEPO       : $('#NOPO').val().split(" | ")[1],
...
function reset(status_reset=""){
	...
	if(status_reset != "NOPO")
	{
		...
		$("#NOPO").val($('#NOPO option:eq(0)').val()); //DEFAULT
		$('#NOPO').trigger('change');
	...
	$("#btn_simpan, #btn_tambah,#DETAILBARANG,#btn_search,#btn_salin").css('filter', '');
	$("#btn_simpan, #btn_tambah,#DETAILBARANG,#btn_search,#btn_salin").removeAttr('disabled');
}
...
function numberInput(evt,e,field) {
    ...
    for(var x = 0 ; x < dataPO.length ; x++)
	{
	    if(dataPO[x].KODE == $("#KODE"+e).html())
	    {
	        if(parseInt($("#JUMLAH"+e+" input").val()) > parseInt(dataPO[x].QTY))
	        {
	            $("#JUMLAH"+e+" input").val(dataPO[x].QTY);
	            
	            Swal.fire({
					title            : dataPO[x].NAMA+"\nSisa PO adalah "+parseInt(dataPO[x].QTY)+" "+dataPO[x].SATUANBESAR,
					type             : 'success',
					showConfirmButton: false,
					timer            : 1500
				});
