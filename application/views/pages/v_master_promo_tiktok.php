...
<section class="content">
  	...
  	<div class="modal-content">
  	    <div class="scroll-wrapper">
      		<div class="modal-body">
                  <div class="box-body">
                      <input type="hidden" id="IDPROMOSITIKTOK" name="IDPROMOSITIKTOK">
                      <div class="row">
                          <div class="form-group col-md-12"  style="padding-right:0px;">
                              <div class="row">
                                   <div class="col-md-12" style="padding-right:0px;">
                                        <h3 style="font-weight:bold;" class="form-group" id="titleTiktok">Tambah Promo</h3>
                                    ...
                                    <div class="form-group col-md-12">
                                        ...
                                        <input type="text" class="form-control" id="NAMAPROMOSITIKTOK" name="NAMAPROMOSITIKTOK" placeholder="Nama Produk">
                                    </div>
                              </div>
                              <div class="row">
                                    <div class="form-group col-md-3">
                                        <label>Tgl Mulai <i style="color:grey;">&nbsp;&nbsp;&nbsp;Wajib</i></label>
                                        <input type="text" style="border:1px solid #B5B4B4; border-radius:1px;"  placeholder="Min 1 Jam dari Sekarang" class="form-control datetimepicker-input" id="TGLMULAITIKTOK" name="TGLMULAITIKTOK" data-toggle="datetimepicker" data-target="#TGLMULAITIKTOK"/>
                                	</div>
                                	...
                                	<div class="form-group col-md-3">
                                		...
                                		<input type="text" style="border:1px solid #B5B4B4; border-radius:1px;"  placeholder="Min 1 Jam dari Tgl Mulai" class="form-control datetimepicker-input" id="TGLAKHIRTIKTOK" name="TGLAKHIRTIKTOK" data-toggle="datetimepicker" data-target="#TGLAKHIRTIKTOK"/>
                                	</div>
                           ...
                           <div class="form-group col-md-12">
                                <div class="row">
                                    <div class="col-md-12">
                                      ...
                                      <button type="button" style="float:left; position: relative; z-index: 10;"  id="btn_tambah_produk_tiktok" class="btn btn-success" onclick="javascript:tambahProduk()">Tambah</button>
  ...
  <div class="modal fade" id="modal-barang">
  	...
  	<div class="modal-content">
  		<div class="modal-body">
  		    <button type="button" id="btn_tambah_produk_tiktok" class="btn btn-success" style="position:absolute; right:15px;" onclick="javascript:simpanProduk()">Pilih Produk <span id="countProduk"></span></button>
...
<input type="hidden" id="statusTransTiktok">
...
$(document).ready(function() {
    ...
    $('#TGLMULAITIKTOK').datetimepicker({
      format: 'YYYY-MM-DD HH:mm', // Change to 'L LT' for date + time
      sideBySide: true,
    });
    $('#TGLAKHIRTIKTOK').datetimepicker({
      format: 'YYYY-MM-DD HH:mm',// Change to 'L LT' for date + time
      sideBySide: true,
    });
//   $('#TGLMULAITIKTOK, #TGLAKHIRTIKTOK').timepicker({
//         timeFormat: 'HH:mm:ss'
//     });
	
    $("#statusTiktok").val('ONGOING');
    //MENAMPILKAN TRANSAKSI
    $("#cb_barang_status_tiktok").change(function(event){
        loading();
        $("#statusTiktok").val($(this).val());
        $("#dataGridPromoTiktok").DataTable().ajax.reload();
    });
    
    $('#dataGridPromoTiktok').DataTable({
        'paging'      : true,
        'lengthChange': true,
        'searching'   : true,
        'ordering'    : false,
		...
		"scrollX"	  : true,
		ajax		  : {
			url    : base_url+'Tiktok/dataGridPromo',
			dataSrc: "rows",
			type   : "POST",
			data   : function(e){
		...
		columnDefs: [
			...
			,
			{
                ...
                "render" :function (data,display,row) 
    			{
                	...
                	if(data == "NOT_EFFECTIVE")
                	{
                	    status = "Non Aktif oleh Tiktok";
                	}
                	if(data == "EXPIRED")
                	{
    ...
    //DAPATKAN INDEX
	$('#dataGridPromoTiktok tbody').on( 'click', 'button', function () {
		var row = $('#dataGridPromoTiktok').DataTable().row( $(this).parents('tr') ).data();
		var mode = $(this).attr("id");
		if(mode == "btn_perpanjang"){ ubahTiktok(row,'PERPANJANG');}
		if(mode == "btn_ubah"){ ubahTiktok(row,'UBAH'); }
		if(mode == "btn_hapus"){ hapusTiktok(row); }
	});
    
    $('#dataGridPromoTiktok').DataTable().on('xhr.dt', function () {
        Swal.close();
    });
    ...
    $("#TGLMULAITIKTOK").on('change.datetimepicker', function(e) {
        if($("#modeTiktok").val() == 'TAMBAH')
        {
            $('#dataGridVarianPromo').DataTable().clear().draw();
        }
    });
    
    $("#TGLAKHIRTIKTOK").on('change.datetimepicker', function(e) {
        if($("#modeTiktok").val() == 'TAMBAH')
        {
            $('#dataGridVarianPromo').DataTable().clear().draw();
        }
    ...
    $('#dataGridVarianPromo').DataTable({
    	...
    	"scrollX"	  : true,
//     	ajax		  : {
//     		url    : base_url+'Tiktok/dataGridPromoBarang',
//     		dataSrc: "rows",
// 			type   : "POST",
//     	},
        columns:[
            ...
            {data: 'MODE',width:0},
            {data: 'IDINDUKBARANGTIKTOK', visible:false},
            {data: 'IDBARANGTIKTOK', visible:false},
		...
		ajax		  : {
        	...
        	dataSrc: function(json) {
                ...
                json.rows = json.rows.filter(function(item) {
                    return item.IDINDUKBARANGTIKTOK != 0;
                });
                return json.rows;
            },
			type   : "POST",
    ...
    //GET DATA ALL
     $.ajax({
    	...
    	success : function(msg){
                   ...
                   if(dataBarang[x].KATEGORI == kategori)
                   {
                        ...
                        if(!ada)
                        {
                            var newRow = {
                                'MODE'                  : 'TAMBAH',
                                'ID'                    : dataBarang[x].ID,
                                'IDINDUKBARANGTIKTOK'   : dataBarang[x].IDINDUKBARANGTIKTOK,
                                'IDBARANGTIKTOK'        : dataBarang[x].IDBARANGTIKTOK,
                            ...
                            listIDBarang[listIDBarang.length-1] = dataBarang[x].IDINDUKBARANGTIKTOK;
            ...
            $.ajax({
            	type    : 'POST',
            	url     : base_url+'Tiktok/getItemPromo',
            	data    : {databarang: JSON.stringify(listIDBarang), tglmulai:$("#TGLMULAITIKTOK").val(),tglakhir:$("#TGLAKHIRTIKTOK").val()},
            	dataType: 'json',
            	success : function(msg){
            	    var dataBarang = msg.rows;
                    for(var x = 0 ; x < dataBarang.length;x++)
...
function tambahProduk(){
    if($("#TGLMULAITIKTOK").val() != "" && $("#TGLAKHIRTIKTOK").val() != "")
    {
        const tglMulai = moment($('#TGLMULAITIKTOK').val(), 'YYYY-MM-DD HH:mm');
        const tglAkhir = moment($('#TGLAKHIRTIKTOK').val(), 'YYYY-MM-DD HH:mm');
...
function tambahTiktok(){
    reset();
    $("#titleTiktok").html("Tambah Promo");
    $("#modeTiktok").val("TAMBAH");
    $("#modal-barang-tiktok").modal('show');
}

function ubahTiktok(row,title){
    reset();
    loading();
    $("#IDPROMOSITIKTOK").val(row.IDPROMOSI);
    $("#NAMAPROMOSITIKTOK").val(row.NAMAPROMOSI);
    
    
    if(title == "UBAH")
    {
        $("#statusTransTiktok").val(row.STATUS);
        $("#titleTiktok").html("Ubah Promo");
        ...
        $("#NAMAPROMOSITIKTOK").attr('disabled','disabled');
        $("#TGLMULAITIKTOK").attr('disabled','disabled');
        $("#TGLAKHIRTIKTOK").attr('disabled','disabled');
        
        
        $("#TGLMULAITIKTOK").val(row.TGLMULAI);
        $("#TGLAKHIRTIKTOK").val(row.TGLAKHIR);
    
    }
    else if(title == "PERPANJANG")
    {
        $("#statusTransTiktok").val('upcoming');
        $("#titleTiktok").html("Perpanjang Promo");
        
        $("#TGLMULAITIKTOK").val(row.TGLAKHIR);
        $("#TGLAKHIRTIKTOK").val("");
    }
    
    
    $("#modeTiktok").val(title);
    $("#modal-barang-tiktok").modal('show');
    //GET DATA ALL
    $.ajax({
    	type    : 'POST',
    	url     : base_url+'Tiktok/getPromo',
    	...
    	success : function(msg){
    	    for(var x = 0 ; x < msg.rows.length; x++)
    	    {
                ...
                var table = $('#dataGridVarianPromo').DataTable();
                var rows = {
                    'MODE'                  : ($("#modeTiktok").val() == "UBAH" ? 'UBAH' : 'TAMBAH'),
                    'ID'                    : rowData.ID,
                    'IDINDUKBARANGTIKTOK'   : rowData.IDINDUKBARANGTIKTOK,
                    'IDBARANGTIKTOK'        : rowData.IDBARANGTIKTOK,
                ...
                $("#mode_"+rowData.ID).val(($("#modeTiktok").val() == "UBAH" ? 'UBAH' : 'TAMBAH'));
...
function simpanTiktok(){
    ...
    if($("#NAMAPROMOTIKTOK").val() == 0 || $("#TGLMULAITIKTOK").val() == ""  || $("#TGLAKHIRTIKTOK").val() == ""  || count == 0)
    {
        Swal.fire({ 
    ...
    else
    {
        ...
        if (tglMulai.isAfter(tglAkhir)) {
          Swal.fire({ 
            ...
            $.ajax({
            	type    : 'POST',
            	url     : base_url+'Tiktok/setPromo/',
            	data    : {
            	   "mode"            : $("#modeTiktok").val(),
            	   "idpromosi"       : $("#IDPROMOSITIKTOK").val(),
            	   "namapromosi"     : $("#NAMAPROMOSITIKTOK").val(), 
            	   "tglmulai"        : $("#TGLMULAITIKTOK").val(), 
            	   "tglakhir"        : $("#TGLAKHIRTIKTOK").val(), 
            	   "status"          : $("#statusTransTiktok").val(),
...
function hapusTiktok(row){
   Swal.fire({
        title: 'Anda Yakin Akan Mengakhiri Promo ini di Tiktok <br>'+row.NAMAPROMOSI+' ?',
        ...
        /* Read more about isConfirmed, isDenied below */
        	if (result.value) {
        	     ...
        	     $.ajax({
                	type    : 'POST',
                	url     : base_url+'Tiktok/removePromo/',
...
function reset(){
    ...
    $('#dataGridVarianPromo').DataTable().clear().draw();
    $("#NAMAPROMOSITIKTOK").val("");
    $("#IDPROMOSITIKTOK").val("");
    $("#TGLMULAITIKTOK").val("");
    $("#TGLAKHIRTIKTOK").val("");
    $("#NAMAPROMOSITIKTOK").removeAttr("disabled");
    $("#TGLMULAITIKTOK").removeAttr("disabled");
    $("#TGLAKHIRTIKTOK").removeAttr("disabled");
    $("#persenAll").val("");
    $("#batasAll").val("");
    $("#pilihDetailProdukSemua").prop('checked', false);
    $("#statusTransTiktok").val("upcoming");
