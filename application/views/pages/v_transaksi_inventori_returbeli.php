...
$(document).ready(function(){
    ...
    $("#btn_salin").click(function(){
        if($("#DETAILBARANG").val() != "")
        {
    	    $.ajax({
    			type    : 'POST',
    			url     : base_url+'Inventori/Transaksi/ReturPembelian/loadDetail/',
	...
	$('#dataGrid').DataTable({
		...
		ajax		  : {
			url    : base_url+'Inventori/Transaksi/ReturPembelian/dataGrid',
			dataSrc: "rows",
			type   : "POST",
			data   : function(e){
					e.status 		 = getStatus();
        ...
        columns:[
            { data: '' },    
            { data: 'IDRETURBELI', visible: false,},	
            { data: 'TGLTRANS', className:"text-center"},	
            { data: 'KODERETURBELI', className:"text-center"},
...
function ubah(row){
	...
	get_akses_user('<?=$_GET['kode']?>', function(data){
		if (data.UBAH==1) {
			...
			get_status_trans("Inventori/Transaksi/ReturPembelian",row.IDRETURBELI,function(data){
			...
			//HEADER
			 $.ajax({
				type    : 'POST',
				url     : base_url+'Inventori/Transaksi/ReturPembelian/loadHeader/', 
				data    : {id:row.IDRETURBELI,mode:"ubah"},
				dataType: 'json',
				success: function(msg){
        			...
        			$.ajax({
        				...
        				success : function(msg){			
        						...
        						//$("#kode-tgl").html(row.KODERETURBELI+" &nbsp; ~ &nbsp; "+row.TGLTRANS+"<br>"+row.NAMASUPPLIER);
		...
		if (data.status=='I' || data.status=='S') {
			get_akses_user('<?=$kodemenu?>', function(data){
				if (data.HAPUS==1) {
					...
					$("#KETERANGAN_BATAL").html("Apa anda yakin akan membatalkan transaksi "+row.KODERETURBELI+" ?");
				} else {
					Swal.fire({
						title            : 'Anda Tidak Memiliki Hak Akses',
						type             : 'error',
...
function batal(){
	...
	if (row  && alasan != "") {
		$.ajax({
			type    : 'POST',
			dataType: 'json',
			url     : base_url+"Inventori/Transaksi/ReturPembelian/batalTrans",
			data    : "idtrans="+row.IDRETURBELI + "&kodetrans="+row.KODERETURBELI + "&alasan="+alasan,
			cache   : false,
			success : function(msg){
				
				if (msg.success) {
					Swal.fire({
						title            : 'Transaksi dengan kode '+row.KODERETURBELI+' telah dibatalkan',
						type             : 'success',
						showConfirmButton: false,
						timer            : 1500
					});
			...
			get_status_trans("Inventori/Transaksi/ReturPembelian",row.IDRETURBELI, function(data){
				if (data.status =='I') {
					$.ajax({
						type    : 'POST',
						dataType: 'json',
						url     : base_url+'Inventori/Transaksi/ReturPembelian/ubahStatusJadiSlip',
						data    : {idtrans: row.IDRETURBELI, kodetrans: row.KODERETURBELI},
						cache   : false,
						success : function(msg){
							if (msg.success) {
								$("#dataGrid").DataTable().ajax.reload();
								window.open(base_url+"Inventori/Transaksi/ReturPembelian/cetak/"+row.IDRETURBELI, '_blank');
							} else {
								Swal.fire({
									title            : msg.errorMsg,
									type             : 'error',
...
function simpan(){
	...
	else
	{
		$.ajax({
			type    : 'POST',
			url     : base_url+'Inventori/Transaksi/ReturPembelian/simpan/', 
			data    : {	IDRETURBELI	 : 	$("#IDTRANS").val(),
						KODERETURBELI: $("#NOTRANS").val(),
						IDLOKASI     : $("#LOKASI").val(),
